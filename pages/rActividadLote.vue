...
export default {
  ...
  methods: {
    ...
    async readActivdadLote() {
      this.mListInsumoLotes = [];
      this.loadingInsumoLotes = true;
      try {
        var datos = await this.$axios.post(
          process.env.baseUrl + "/RActividadLote",
          {
            token: this.token,
            mercados: this.mSelectLote.length == 0 ? "*" : this.mSelectLote,
          }
        );
    ...
    async exportPdfRSalidas() {
      ...
      //CodiVehiDispEven,HoraDispEven,DescRutaSali_m,NumeVuelSali_m,DescFrec,DescDispEvenList,LatiDispEven,LongDispEven
      var idLoteAnterior = null;

      for (var i = 0; i < this.mListInsumoLotes.length; i++) {
        var insumoLote = this.mListInsumoLotes[i];

        if (insumoLote.id_lote !== idLoteAnterior) {
          resultadoString.push([{ text: "\n", colSpan: 4 }]); // Agregar salto de línea
          idLoteAnterior = insumoLote.id_lote; // Actualizar el id_lote anterior
        }

        var arrys = [
          {
            text: insumoLote.nombre_lote,
            alignment: "center",
            fontSize: 8.5,
          },
          {
            text: insumoLote.nombre_mercado,
            fontSize: 8.5,
            alignment: "center",
          },
          {
            text: insumoLote.contador,
            fontSize: 8.5,
            alignment: "center",
          },
          {
            text: insumoLote.detalle_actividad,
            fontSize: 8.5,
            alignment: "center",
          },
        ];
